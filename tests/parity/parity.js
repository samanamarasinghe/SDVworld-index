...
function readPage(doc) {
  ...
  const groups = [...results.querySelectorAll('.pub-group')].map(sec => ({
    ...
    titles: [...sec.querySelectorAll('li.pub-item .pub-title')].map(txt),
  }));
  return {
    count: txt(doc.getElementById('pubs-count')),
    grouped: groups.length > 0,
    groups,
    /* Flat view only; under grouping the DOM order interleaves sections and the
       per-group lists above are the meaningful comparison. */
    titles: groups.length ? null
      : [...results.querySelectorAll('li.pub-item .pub-title')].map(txt),
    facets: Object.fromEntries(FACET_MOUNTS.map(id => [
      id, [...(doc.getElementById(id) || document.createElement('div'))
        .querySelectorAll('.facet-text')].map(txt),
    ])),
...
function compare(a, b) {
  ...
  if (!a.grouped) {
    /* v2 renders at most 100 unique records; v1 renders all of them. v2's page must
       be exactly v1's first 100, in order -- which is a stronger check than
       comparing sets, because it also proves the sort agrees. */
    const k = b.titles.length;
    if (k > 100) return `v2 rendered ${k} cards, above its own cap of 100`;
    const want = a.titles.slice(0, k);
    if (!eq(want, b.titles)) {
      const at = want.findIndex((t, i) => t !== b.titles[i]);
      return at < 0
        ? `card list length: v1 prefix ${want.length}, v2 ${b.titles.length}`
        : `card ${at + 1}: v1 "${want[at]}", v2 "${b.titles[at]}"`;
    }
    if (a.titles.length < k) return `v1 rendered ${a.titles.length} cards, v2 ${k}`;
  } else {
    const an = a.groups.map(g => g.name), bn = b.groups.map(g => g.name);
    /* A group can be entirely off v2's page, in which case v2 omits the section
       rather than drawing an empty one. So v2's headings must be a subsequence of
       v1's, in the same relative order. */
    let i = 0;
    ...
    for (const g2 of b.groups) {
      ...
      const want = g1.titles.slice(0, g2.titles.length);
      if (!eq(want, g2.titles)) {
        const at = want.findIndex((t, j) => t !== g2.titles[j]);
        return `group "${g2.name}" card ${at + 1}: v1 "${want[at]}", v2 "${g2.titles[at]}"`;
      }
    }
    /* Every v1 group that v2 omitted must genuinely have had no record on v2's page.
       This is what would catch v2 dropping a whole section by mistake. */
    const shown = new Set(bn);
    const missing = a.groups.filter(g => !shown.has(g.name));
    const rendered = b.groups.reduce((n, g) => n + g.titles.length, 0);
  ...
  for (const id of FACET_MOUNTS) {
    if (!eq(a.facets[id], b.facets[id])) {
      ...
      return `${id} item ${at + 1}: v1 "${x[at]}", v2 "${y[at]}"`;
    }
  }
